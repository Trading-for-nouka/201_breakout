...
# スコア(PF×勝率): 0.8756
# 勝率: 50.8% | PF: 1.73 | 取引数: 1,718
# RS上限追加後  : RSレンジ 3%〜7% に限定（グリッドサーチ最適化済み）

STRATEGY_PARAMS = {

    # ============================================================
    # ブレイクアウト戦略（scan.py）
    # 実績: 勝率50.8% / PF1.73 / 保有21日
    # 最優秀パラメータ: 7日高値ブレイク / MA20>MA50 / RS3〜7% / RVOL≥2.0
    # ============================================================
    "breakout": {
        # エントリーゾーン（ATRベース）
        ...
        # RSフィルター（バックテスト最適化済み）
        "rs_threshold":     0.03,   # RS下限: TOPIXより+3%以上アウトパフォーム
        "rs_threshold_max": 0.07,   # RS上限: TOPIXより+7%未満（過熱防止）

        # バックテスト実績（RS 3〜7%フィルター適用後）
        "win_rate":         0.508,
        "profit_factor":    1.725,
    },
}
